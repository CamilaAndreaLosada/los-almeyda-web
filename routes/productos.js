...
// Ruta para obtener todos los productos con filtrado opcional por categoría
...
router.post('/', verifyToken, isAdmin, async (req, res) => {
    const { nombre, descripcion, precio, stock, id_categoria, imagen_url, unidad } = req.body;
    ...
    try {
        const query = 'INSERT INTO PRODUCTOS (nombre, descripcion, precio, stock, id_categoria, imagen_url, unidad) VALUES (?, ?, ?, ?, ?, ?, ?)';
        const [result] = await db.query(query, [nombre, descripcion, precio, stock, id_categoria, imagen_url || null, unidad || 'lb']);
...
router.put('/:id', verifyToken, isAdmin, async (req, res) => {
    ...
    try {
        const query = 'UPDATE PRODUCTOS SET nombre = ?, descripcion = ?, precio = ?, stock = ?, id_categoria = ?, imagen_url = ?, unidad = ? WHERE id_producto = ?';
        const [result] = await db.query(query, [nombre, descripcion, precio, stock, id_categoria, imagen_url || null, unidad || 'lb', id]);
